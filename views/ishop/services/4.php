<script type="text/javascript">
	...
	function calc_silk() {
		...
		e = (parseInt('<?=$get_service["textileprint_price_4"]?>') + d) * c * a;	

		if (b==''){
			if (isNaN(e)){
				$('span#result_silk').parent().text('Обратитесь для расчета в офис!');
				$('span#result_silk_full').parent().text('Обратитесь для расчета в офис!');
			}else{
				$('span#result_silk').text(e.toFixed(1));			
				$('span#result_silk_full').text((e.toFixed(1)*vars[2]).toFixed(1));		
			}			
		}else{
			$('span#result_silk').text((2500/vars[2]*c).toFixed(1));
			$('span#result_silk_full').text(2500*c).toFixed(1);
		}
	}
			...
			<form method="POST" class="serv_calc_form form-horizontal" action="" id="color_termoprint_form">
				...
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Стоимость за единицу нанесения: <span class="calc_result" id="result_color_termoprint">0</span> руб.</strong> 
						<button class="calc_button" type="submit" onclick="calc_color_termoprint();">Рассчитать</button>
					</div>
				</div>
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Полная стоимость нанесения: <span class="calc_result" id="result_color_termoprint_full">0</span> руб.</strong> 
						<button class="calc_button" type="button" onclick="reset_form_color_termoprint();">Очистить форму</button>
					</div>
				</div>
			</form>		    	
		</div>
		<p class="serv_tip">Для получения качественного изображения (переноса) ткань должна содержать не менее 50% хлопка.</p>
	...
	<div class="serv_content">
		...
		<div class="serv_calc">
			<form method="POST" class="serv_calc_form form-horizontal" action="" id="onecolor_termoprint_form">
				...
				<div class="calc_row">
					<span class="calc_label">Длина в сантиметрах: </span><input type="text" name="onecolor_termoprint_a">
				</div>
				<div class="calc_row">
					<span class="calc_label">Ширина в сантиметрах: </span><input type="text" name="onecolor_termoprint_b">
				</div>
				<div class="calc_row">
				...
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Стоимость за единицу нанесения: <span class="calc_result" id="result_onecolor_termoprint">0</span> руб.</strong> 
						<button class="calc_button" type="submit" onclick="calc_onecolor_termoprint();">Рассчитать</button>
					</div>
				</div>
				<div class="calc_row">
					<div class="calc_result_row">
					<strong>Полная стоимость нанесения: <span class="calc_result" id="result_onecolor_termoprint_full">0</span> руб.</strong> 
						<button class="calc_button" type="button" onclick="reset_form_onecolor_termoprint();">Очистить форму</button>
					</div>
				</div>
			</form>		    	
		</div>
		<p class="serv_tip">Используемый текстиль должен выдерживать температуру не менее 170°C</p>
		<p class="serv_tip">Если наносимое изображение содержит большое количество мелких элементов - наценка 20-40% за сложность.</p>
	...
	<div class="serv_content">
		...
		<div class="serv_calc">
			<form method="POST" class="serv_calc_form form-horizontal" action="" id="silk_form">
				...
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Полная стоимость нанесения: <span class="calc_result" id="result_silk_full">0</span> руб.</strong> 
						<button class="calc_button" type="button" onclick="reset_form_silk();">Очистить форму</button>
					</div>
				</div>
			</form>		    	
		</div>
		<p class="serv_tip">При необходимости получить более плотный слой краски в нанесении все элементы изображения катаются в 2 прохода. Стоимость работ увеличиватся на 15%.</p>
		<p class="serv_tip">При нанесении изображения на «сложную» поверхность (например: толстые ватники, резиновые изделия, рукава и т.д.) стоимость работ увеличивается на 10-30%.</p>
		<p class="serv_tip">При нанесении нескольких цветов возможна наценка 10-20% за сложность совмещения элементов изображения.</p>
		<p class="serv_tip">Файл с изображением, предоставляемый для нанесения должен быть векторным. При отсутствии соответствующего файла стоимость подготовки макета 300-1000 руб.</p>
		<p class="serv_tip">Возможно нанесение изображения нестандартными красками: световозвращающими, пластизольными, вспененными, содержащими блёстки и т.п. Стоимость подобных работ рассчитывается индивидуально.</p>
	</div>
</div>
<hr>
<div class="serv-section" id="sublime">
	...
	<div class="serv_content">
		<div class="serv_preview">
			<p class="serv_desc">Нанесение полноцветного изображения на белую синтетическую ткань методом сублимации.</p>
		</div>
		<div class="serv_calc">
			<form method="POST" class="serv_calc_form form-horizontal" action="" id="sublime_form">
				...
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Стоимость за единицу нанесения: <span class="calc_result" id="result_sublime">0</span> руб.</strong> 
						<button class="calc_button" type="submit" onclick="calc_sublime();">Рассчитать</button>
					</div>
				</div>
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Полная стоимость нанесения: <span class="calc_result" id="result_sublime_full">0</span> руб.</strong> 
						<button class="calc_button" type="button" onclick="reset_form_sublime();">Очистить форму</button>
					</div>
				</div>
			</form>		    	
		</div>
		<p class="serv_tip">Стандартный формат печати - А4. При нанесении изображения формата А5 и менее -30% от стоимости.</p>
		<p class="serv_tip">Для получения качественного изображения (переноса) ткань должна содержать не менее 50% полиэстера.</p>
		...
		<p class="serv_tip">
			<span class="item_gallery">
				<span class="item_thumbs">
					<a rel="gallery" title="Бандана универсальная с сублимацией" href="/userfiles/service_img/baf/1.jpg"><img src="/userfiles/service_img/baf/1.jpg" /></a>
				</span>
			</span>
		</p>
	</div>
...
<div class="serv-section" id="directprint">
	...
	<div class="serv_content">
		<div class="serv_preview">
			<div>
				<p class="serv_desc">Прямая печать полноцветного изображения на белой х/б ткани.</p>
				<p class="serv_desc">Стоимость печати формата А4 - 300 руб., от 30 шт. - 250 руб.</p>
			</div>
		</div>
	</div>
</div>
	...
	<div class="serv_content">
		...
		<div class="serv_calc">
			<form method="POST" class="serv_calc_form form-horizontal" action="" id="baseballprint_form">
				...
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Стоимость за единицу нанесения: <span class="calc_result" id="result_baseballprint">0</span> руб.</strong> 
						<button class="calc_button" type="submit" onclick="calc_baseballprint();">Рассчитать</button>
					</div>
				</div>
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Полная стоимость нанесения: <span class="calc_result" id="result_baseballprint_full">0</span> руб.</strong> 
						<button class="calc_button" type="button" onclick="reset_form_baseballprint();">Очистить форму</button>
					</div>
				</div>
			</form>		    	
		</div>
...
<div class="serv-section" id="hats">
	...
	<div class="serv_content">
		<div class="serv_preview">
			<div>
				<p class="serv_desc">Треугольный платок со сторонами 100х60х60 см из полиэстера или атласа с полноцветной печатью и обрезкой края термоножом.</p>
			</div>
		</div>
		<div class="serv_calc">
			<form method="POST" class="serv_calc_form form-horizontal" action="" id="hats_form">
				...
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Стоимость готового изделия: <span class="calc_result" id="result_hats">0</span> руб.</strong> 
		...
		<p class="serv_tip">Для снижения стоимости изделия возможно изготовление однотонного галстука с небольшим логотипом только в одном углу. Этот и прочие нестандартные варианты рассчитываются индивидуально</p>
	</div>
</div>
<hr>
	...
	<div class="serv_content">
		...
		<div class="serv_calc">
			<form method="POST" class="serv_calc_form form-horizontal" action="" id="bandanas_form">
				...
				<div class="calc_row">
					<div class="calc_result_row">
						<strong>Полная стоимость: <span class="calc_result" id="result_bandanas_full">0</span> руб.</strong> 
						<button class="calc_button" type="button" onclick="reset_form_bandanas();">Очистить форму</button>
					</div>
				</div>
			</form>		    	
		</div>
